...
void loop() {
  ...
  switch (SIM_state)
  {
    ...
    case SENSOR:
      ...
      //introduce an average function
      Serial.print(F("---> Level: "));
      Serial.println(level);
      dtostrf(level, 2, 0, charVal);
      Serial.print(F("---> Level(as char): "));

      for(int i=0; i<sizeof(charVal); i++){
        Serial.print(charVal[i]);
        ///char charSensor[i] = charVal[i];
      }
      Serial.println();
    ...
    case SENDPARA:
     ...
     /*previous data stream smability.com
     ...
     */
     ...
     //snprintf(aux_str, sizeof(aux_str), "AT+HTTPPARA=\"URL\",\"http://smability.sidtecmx.com/Device//SetURL?temp=%s&deviceID=175124decfe59d2116cd5a990e9d9c0e \"",charVal);
    
     //token (temperature) : 175124decfe59d2116cd5a990e9d9c0e 
     //serial: 59320090103512478
     //platform returns: ok!

     //token (battery): a8356741d8fdec2f14db4791074810dc
     //serial: 54720090713150639
     //platform returns: ok!

     snprintf(aux_str, sizeof(aux_str), "AT+HTTPPARA=\"URL\",\"http://smability.sidtecmx.com/Device//SetURL?batt=%s&deviceID=a8356741d8fdec2f14db4791074810dc \"",charVal);
